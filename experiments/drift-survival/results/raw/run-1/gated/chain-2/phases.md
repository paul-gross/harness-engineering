...
- An admin can toggle `exports-enabled` via the admin settings API; the change is reflected on subsequent requests.
...
- Polling the status URL reflects the current job status.
...
- A user without access to the report cannot obtain an export of it; the request is rejected at the API layer before any job is created.
