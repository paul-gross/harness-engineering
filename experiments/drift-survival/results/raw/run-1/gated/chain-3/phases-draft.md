...
- After three consecutive delivery failures the schedule status transitions to `paused` with `pause_source = system`; a `schedule_paused_by_system` audit event is recorded; the `ExportRun` is marked `failed`; no further jobs are dispatched for that schedule.
...
- Audit completeness review: verify all significant export events are captured; add any missing event types (e.g. schedule creation, schedule deletion if supported)
