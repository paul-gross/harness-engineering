...
- **Feature flag / workspace settings** — a lightweight workspace-scoped configuration layer. The feature flag is off by default at launch. Before any export is initiated (on-demand or scheduled dispatch), the Export Service checks the workspace's export-enabled flag. When disabled, the request is rejected immediately and no job is enqueued.
