...
- [ ] An exported file contains only rows the requesting user is permitted to see in the app (row-level permission enforcement is applied at export time, not at report definition time).
...
- [ ] A user can view a history of the last 30 export runs for each of their schedules, including delivery status and any failure reason.
...
- [ ] Every export event (export requested, export succeeded, export failed, retry attempted, schedule paused by system) is recorded in the audit log with the acting user, timestamp, and relevant identifiers.

### Launch

- [ ] The entire feature is gated behind a feature flag and is off by default at launch.
- [ ] Enabling or disabling the feature flag does not require a deployment.
