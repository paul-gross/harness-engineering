...
**Feature flag layer** — a workspace-level gate that the export service and scheduler service check before processing any request. When the flag is disabled by an admin, all export endpoints and scheduled jobs are inert.
...
2. The job is re-enqueued for retry. Steps repeat up to a maximum of 3 attempts.
3. If the third attempt fails, the ExportSchedule transitions to `paused`, the run record is finalized as `failure`, and a notification email is sent to the schedule owner explaining the automatic pause.
