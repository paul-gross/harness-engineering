...
- Implement the retry-and-pause lifecycle in the scheduler service:
  ...
  - On third failure, transition `ExportSchedule` to `paused`, finalize `ScheduleRunRecord` as `failure`, and send the owner a notification email explaining the automatic pause.
...
- On third consecutive failure, the `ExportSchedule` transitions to `paused`, the `ScheduleRunRecord` is finalized with outcome `failure`, and the owner receives a pause notification email.
