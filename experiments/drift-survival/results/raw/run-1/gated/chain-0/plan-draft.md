...
- If an export delivery fails, it is retried automatically, with at most 3 total attempts (1 original + 2 retries) before giving up.
- If all 3 delivery attempts fail, the schedule is automatically paused and the owner is notified.
...
- When exports are disabled for a workspace, all export and scheduling UI is hidden or blocked for users in that workspace, and any attempt to trigger an export via API returns an appropriate error.

### Audit logging

- Every export event (one-off or scheduled, success or failure, initiation and completion) is recorded in the audit log.
- Audit log entries capture at minimum: user identity, report identifier, export format, timestamp, and outcome.
