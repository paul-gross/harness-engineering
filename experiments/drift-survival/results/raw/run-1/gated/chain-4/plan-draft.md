...
- Each export event (initiated, completed, failed, rejected) is written to the audit log with the user identity, report identity, format, timestamp, and outcome.
...
- If the third retry attempt fails, the schedule is automatically paused and the owner is notified.
- Each scheduled export run is subject to the same permission enforcement and size limit as on-demand exports.
- Each schedule exposes a run history showing the last 30 export runs, including status (delivered, failed, retried, rejected) and timestamp for each.

### Workspace and platform constraints

- The entire export feature (on-demand and scheduled) is gated behind a feature flag at launch; it is off by default and must be explicitly enabled per environment or cohort.
...
- When export is disabled at the workspace level, users see a clear indication that the feature is unavailable rather than an error or empty state.
