...
**Delivery service** — handles email delivery of completed scheduled export files to the schedule owner. Accepts a completed export artifact and a schedule record, sends to the schedule owner, and reports success or failure back to the schedule manager for retry accounting.
...
**ExportSchedule** — represents a recurring export configuration. Attributes: id, workspace id, owner user id, report id, format, frequency (daily | weekly), state (active | paused), created at, updated at, paused at (nullable).

**ScheduledRun** — one execution of a schedule. Attributes: id, schedule id, scheduled at, started at (nullable), completed at (nullable), status (pending | processing | delivered | failed | abandoned), attempt count, delivery status detail. Retained for the last 30 runs per schedule; older records are pruned.
...
11. The user polls or is notified that the export is ready or failed.
...
7. Schedule manager evaluates retry eligibility: if attempt count < 3, re-enqueue with backoff.
8. If attempt count reaches 3 and delivery still fails, ScheduledRun is marked abandoned, the ExportSchedule is set to paused, and an audit event is written.

**Pause and resume**

1. Owner requests pause of a schedule.
2. Export service verifies the requestor is the schedule owner.
3. ExportSchedule state is set to paused; no future runs are emitted until resumed.
...
- When the flag is disabled, export endpoints return a disabled response; no data is deleted. Re-enabling restores full functionality.
...
- Get workspace export config: returns current enabled state.
...
- Worker → Report query engine: request report data for a given report id and user id; engine applies row-level permission filtering and returns a result set.
