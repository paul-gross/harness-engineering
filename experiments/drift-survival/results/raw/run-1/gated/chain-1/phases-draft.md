...
- Auto-pause: on third consecutive delivery failure, mark `ScheduledRun` as `abandoned`, set `ExportSchedule` to `paused`, write `schedule.run.abandoned` audit event.
...
- Documentation: API contracts, internal interface contracts, and retry/auto-pause behavior documented for operators.
...
- Retry sequence tested end-to-end: three consecutive delivery failures result in `abandoned` run and `paused` schedule with all expected audit events.
