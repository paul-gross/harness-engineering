...
- Define and implement the audit log entity and logging interface with action types: "export_created", "export_completed", "export_failed", "schedule_created", "schedule_paused", "schedule_resumed", "delivery_attempted", "delivery_succeeded", "delivery_failed"
...
- Build the Scheduler Service: detects when a schedule's trigger time arrives, enqueues Export Jobs linked to the schedule, applies schedule-configured format (defaults to CSV)
...
- The user can inspect individual Export Delivery Attempts to understand why delivery failed
