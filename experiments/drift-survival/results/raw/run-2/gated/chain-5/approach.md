...
2. **Schedule Manager** — owns the lifecycle of recurring export schedules (creation, pause/resume, deletion, history tracking). Schedules are owned by individual users.
...
4. **Delivery Engine** — handles transport and retry logic for scheduled exports; emails are the primary delivery channel. It implements retry logic up to 3 total attempts per schedule run, then auto-pauses the schedule if all retries fail.
...
7. **Storage Layer** — holds generated export artifacts temporarily and maintains schedule definitions, run history, and retry state. Run history is limited to the last 30 runs per schedule.
...
- `artifact_url`: signed URL for download
- `generation_started_at`: timestamp
- `generation_completed_at`: timestamp
...
6. The request record remains for audit.
...
4. The schedule is immediately registered in the scheduler for the next run slot.
...
6. Retry logic (on delivery failure):
   - If `delivery_attempts` < 3, schedules a retry.
...
7. Scheduler calculates next run time and updates `next_run_at`.
...
4. System displays the last 30 Export Runs, showing:
   - Run timestamp
   - Status
   - Artifact size
   - Delivery status and any error messages
5. User can download a generated artifact directly from the history or view error details.
...
6. System sets `is_paused` = false and recalculates `next_run_at`. Audit log records the resume event.
...
**generateExport(reportId, userId, exportFormat) -> ExportArtifact**
...
- Request body: { reportId, format: "CSV" | "PDF", frequency: "DAILY" | "WEEKLY" }
- Returns: { scheduleId, ownerId, reportId, format, frequency, isPaused, createdAt, nextRunAt }
- Side effects: creates Export Schedule record, registers with scheduler, records audit event
- HTTP 400 if frequency/format invalid; 403 if disabled workspace-wide; 404 if report not found
...
- Side effects: deletes Export Schedule, records audit event, cancels pending runs
- HTTP 403 if user is not the schedule owner; 404 if schedule not found
...
- Downloads a previously generated artifact from run history
- Returns: binary artifact data with appropriate Content-Type header
- HTTP 403 if user is not the schedule owner; 404 if run not found
