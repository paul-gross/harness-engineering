...
- Permission Enforcer correctly filters rows based on user access rules
...
- ScheduledExport retention policy implementation supports keeping last 30 runs
...
- Add size validation with 50 MB limit (reject exports exceeding this size)
...
- Exports exceeding 50 MB are rejected with user-facing error message
...
- Wire scheduled export creation audit logging including schedule pause and resume events
- Calculate and store first execution time when schedule is created
- Implement pause/resume functionality with status tracking, pause timestamps, and pause-by-user-ID recording
...
- Pause action updates status, records pause timestamp and user ID, and prevents future executions
...
- Implement delivery retry logic with up to 3 attempts and scheduled retries
- Create ExportRun records to track each execution with status, attempt count, timestamps, and next retry scheduled time
- Implement automatic pause on 3 failed delivery attempts, recording pause_at timestamp and pause_by user ID
- Wire Audit Logger to record export_generated, export_delivered, export_failed, and schedule pause events
- Build scheduler integration to trigger periodic execution
- Handle SMTP failures and invalid email addresses gracefully
- Apply 50 MB size limit to scheduled export generation
...
- Schedule is auto-paused after 3 failed delivery attempts with pause timestamp and paused-by user ID recorded
- ExportRun records accurately reflect execution status, timestamps, and attempt counts
- ExportRun records include next retry scheduled time when applicable
- Audit log contains complete records of generation, delivery attempts, and failures
- Export generation respects 50 MB size limit and rejects oversized exports
...
- Implement feature flag disable behavior: skip scheduled executions, hide UI controls, reject export endpoints with 403
...
- Feature disabled state prevents all export endpoints from functioning with 403 response
