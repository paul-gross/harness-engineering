...
These subsystems interact through an asynchronous job queue. The Export Generation Engine enqueues jobs; the Scheduling Service polls the queue and triggers generation at configured intervals; the Delivery & Retry Handler consumes completed exports and manages email delivery with exponential backoff retry semantics.
...
2. Constructs email with download link (or attachment, depending on size)
...
5. On failure (first/second attempt):
   - Retry with exponential backoff (e.g., 1 min, 5 min)
...
- On failure, attemptNumber < 3: enqueues retry with backoff, returns status failed
