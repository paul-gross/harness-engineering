...
- `state` — current state: `active`, `paused`, or `error`
...
- `next_retry_at` — when the next retry will occur (null if no retry scheduled)
...
3. If `attempt_count` < 3:
   a. Schedule the next retry (with exponential backoff)
...
4. If `attempt_count` == 3:
   ...
   b. Schedule transitions to `paused` state with an error marker
