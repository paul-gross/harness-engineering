...
8. Delivery and Retry System receives the GenerationComplete event and attempts immediate delivery via email (or waits for user to manually download if one-off).
...
7. On failure, the Delivery and Retry System schedules a retry with exponential backoff (e.g., 1 hour, then 1 day, then final attempt).
...
- Responsibility: Schedule the next retry attempt with exponential backoff if attempt_number < 3; otherwise notify SchedulePauseManager.
